...
DO $$
    BEGIN
    IF ((SELECT max(schema_version) FROM cws_versions) = 1) THEN
...
DO $$
    BEGIN
    IF ((SELECT max(schema_version) FROM cws_versions) = 2) THEN
        -- First feature release, CWS 1.1.x results requires an update of the DB
        INSERT INTO cws_versions(schema_version, cws_version, db_vendor) VALUES (3, '1.2.0', 'PostgreSQL');

        ALTER TABLE cws_metadata ADD COLUMN size INTEGER DEFAULT 0;
        UPDATE cws_metadata SET size = 0 WHERE size IS NULL;
        ALTER TABLE cws_metadata RENAME CONSTRAINT metadata_notafter_parent_id TO metadata_before_parent_id;
        ALTER TABLE cws_signatures RENAME CONSTRAINT signarure_notnull_id to signature_notnull_id;
        ALTER TABLE cws_signatures RENAME CONSTRAINT signarure_notnull_member_id TO signature_notnull_member_id;
        ALTER TABLE cws_signatures RENAME CONSTRAINT signarure_notnull_altered TO signature_notnull_altered;
        ALTER TABLE cws_signatures RENAME CONSTRAINT signarure_notnull_added TO signature_notnull_added;
    END IF;
    COMMIT;
END $$;
